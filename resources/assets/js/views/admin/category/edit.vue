   ...
   // 编辑分类
    submitForm(formName){
      ...
      this.$refs[formName].validate((valid)=>{
        if(valid){
          this.$http.put(`/admin/category/${this.$route.params.id}`,formData)
            .then(({data})=>{
    ...
    initData(){
      this.$http.get(`/admin/category/${this.$route.params.id}/edit`)
        .then(({data})=>{
          if(data.responData.status === 200){
            this.ruleForm.name = data.responData.category.data.name;
            this.ruleForm.desc = data.responData.category.data.desc;
            this.ruleForm.path = data.responData.category.data.path;
            this.ruleForm.id = data.responData.category.data.id;
            this.ruleForm.pid = data.responData.category.data.pid;
