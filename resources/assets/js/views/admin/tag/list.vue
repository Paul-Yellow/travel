<template>
  <div class="list">
    ...
    <el-table   :data="dataList" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="300px" label="标签名字">
        <template slot-scope="scope">
...
export default {
  ...
  methods:{
    ...
    handleSizeChange(limit){
     this.$router.push({ path:'/admin/tag', query: { limit } })
    },
    handleCurrentChange(page){
      this.$router.push({ path:'/admin/tag', query: { page } })
    },
    // 编辑类型信息
    handleEdit(e){
      let id = e.id;
      this.$router.push({path:`/admin/tag/${id}/edit`});
    },
    // 删除类型
    handleDelete(e){
      this.$http.delete(`/admin/tag/${e.id}`)
        .then((res)=>{
          if(res.status === 204)
            {
              this.$message({
    ...
    fetchList(){
      ...
      this.$http.get(`/admin/tag?page=${this.currentPage}&limit=${this.currentLimit}`)
      .then(({data})=>{
        if(data.data){
          this.list = data.data;
