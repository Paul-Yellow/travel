...
export default {
  ...
  created(){
    this.$http.get(`/admin/newevents/${this.$route.params.id}/edit`)
      .then(({data})=>{
        if(data.responData.status === 200 ){
          this.postForm.content = data.responData.newevents.data.content;
          this.postForm.title = data.responData.newevents.data.title;
          this.postForm.pid = Object.assign({},{id:data.responData.newevents.data.id,name:data.responData.newevents.data.category_name,pid:data.responData.newevents.data.category_id});
          this.postForm.options = data.responData.category.data;
          this.postForm.tagsAll = data.responData.tagAll;
          this.postForm.region = data.responData.newevents.data.area;
          this.postForm.content_short = data.responData.newevents.data.content_short;
          this.imgTmp = data.responData.newevents.data.imgUrl;
        }
      });
  },
  computed: {
    ...
    submitForm(){
      ...
      this.$refs.postForm.validate(valid=>{
        if(valid){
          ...
          this.loading = true;
          this.$http.put(`/admin/newevents/${this.$route.params.id}`,{params:data})
            .then(({data})=>{
              if(data.responData.status === 200){
                this.$message({
                  type: 'success',
