...
export default {
  ...
  created(){
    this.$http.get(`/admin/activeevents/${this.$route.params.id}/edit`)
      .then(({data})=>{
        if(data.responData.status === 200 ){
          ...
          this.postForm.sponsor = data.responData.data.data.sponsor;
    ...
    submitForm(){
      ...
      this.$refs.postForm.validate(valid=>{
        if(valid){
          ...
          this.loading = true;
          this.$http.put(`/admin/activeevents/${this.$route.params.id}`,{params:data})
            .then(({data})=>{
              if(data.responData.status === 200){
                this.$message({
                  type: 'success',
