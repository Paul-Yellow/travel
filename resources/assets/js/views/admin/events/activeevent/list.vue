...
export default {
  ...
  data(){
    return {
      ...
      create: '/admin/events/activeevent/create'
    }
  },
  methods:{
    // 搜索
    ...
    handleSizeChange(limit){
     this.$router.push({ path:'/admin/events/activeevent', query: { limit } })
    },
    handleCurrentChange(page){
      this.$router.push({ path:'/admin/events/activeevent', query: { page } })
    },
    // 编辑活动信息
    handleEdit(e){
      let id = e.id;
      this.$router.push({path:`/admin/events/activeevent/${id}/edit`});
    },
    // 删除活动
    handleDelete(e){
      this.$http.delete(`/admin/activeevents/${e.id}`)
        .then((res)=>{
          if(res.status === 204)
            {
              this.$message({
    ...
    fetchList(){
      ...
      this.$http.get(`/admin/activeevents?page=${this.currentPage}&limit=${this.currentLimit}`)
      .then(({data})=>{
        if(data.data){
          this.list = data.data;
