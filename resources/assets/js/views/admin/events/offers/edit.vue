  ...
  <div class="create">
    <el-row>
      <el-form class="form-container" :model="postForm" :rules="rules" ref="postForm" label-width="100px">
        <el-form-item  label="店家名字:" prop="name">
          <el-col :span="14">
            <el-input type="text" class="article-textarea" placeholder="请输入内容" v-model="postForm.name">
            </el-input>
          </el-col>
        </el-form-item>
        <el-form-item  label="优惠信息:" prop="explain">
          <el-col :span="14">
            <el-input type="text" class="article-textarea" placeholder="请输入内容" v-model="postForm.explain">
            </el-input>
          </el-col>
        </el-form-item>
        
      <el-form-item  label="店家电话:" prop="phone">
          <el-col :span="14">
            <el-input type="text" class="article-textarea" placeholder="请输入内容" v-model="postForm.phone">
            </el-input>
          </el-col>
        </el-form-item>
        ...
        <el-form-item label="店家位置" prop="region">
          <el-col :span="14">
            <el-select v-model="postForm.region" placeholder="请选择店家位置">
...
export default {
  ...
  data(){
    return {
      ...
      loading: false,
      rules: {
        name: [{ required: true, message: '店家名字不能为空', trigger: 'blur,change' }],
        explain: [{ required: true, message: '优惠信息不能为空', trigger: 'blur,change' }],
        phone: [
  ...
  created(){
    this.$http.get(`/admin/offers/${this.$route.params.id}/edit`)
      .then(({data})=>{
        if(data.responData.status === 200 ){
          this.postForm.content = data.responData.data.data.content;
          this.postForm.name = data.responData.data.data.name;
          this.postForm.phone = data.responData.data.data.phone;
          this.postForm.explain = data.responData.data.data.explain;
          ...
          this.postForm.region = data.responData.data.data.area;
    ...
    submitForm(){
      ...
      this.$refs.postForm.validate(valid=>{
        if(valid){
          ...
          this.loading = true;
          this.$http.put(`/admin/offers/${this.$route.params.id}`,{params:data})
            .then(({data})=>{
              if(data.responData.status === 200){
                this.$message({
                  type: 'success',
