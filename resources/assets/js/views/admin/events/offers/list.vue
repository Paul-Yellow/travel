  ...
  <div class="list">
    ...
    <el-table :data="dataList" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column  label="优惠信息">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.explain}}</span>
          <el-tag>{{scope.row.explain}}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  data(){
    return {
      ...
      create: '/admin/events/offers/create'
    }
  },
  methods:{
    // 搜索
    ...
    handleSizeChange(limit){
     this.$router.push({ path:'/admin/events/offers', query: { limit } })
    },
    handleCurrentChange(page){
      this.$router.push({ path:'/admin/events/offers', query: { page } })
    },
    // 编辑活动信息
    handleEdit(e){
      let id = e.id;
      this.$router.push({path:`/admin/events/offers/${id}/edit`});
    },
    // 删除活动
    handleDelete(e){
      this.$http.delete(`/admin/offers/${e.id}`)
        .then((res)=>{
          if(res.status === 204)
            {
              this.$message({
    ...
    fetchList(){
      ...
      this.$http.get(`/admin/offers?page=${this.currentPage}&limit=${this.currentLimit}`)
      .then(({data})=>{
        if(data.data){
          this.list = data.data;
