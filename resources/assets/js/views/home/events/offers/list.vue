      ...
      <ul class="event-news-card-list">
        <li class="item news" v-for="(item,index) in dataList" :key="index">
          <div class="info-card-item">
            <router-link :to="`/offers/detail/${item.id}`" class="link" :title="item.explain">
            ...
            <div class="func-blk">
              ...
              <favourite :item="item" type="favorited" api="/admin/offers/favourite"></favourite>
              <links :item="item" type="liked" api="/admin/offers/links"></links>
            </div>
          </div>
        </li>
      </ul>
...
export default {
  ...
  methods:{
    handleSizeChange(limit){
     this.$router.push({ path:'/offers', query: { limit } })
    },
    handleCurrentChange(page){
      this.$router.push({ path:'/offers', query: { page } })
    },
    fetchList(){
      this.listLoading = true;
      this.$http.get(`/offers?page=${this.currentPage}&limit=${this.currentLimit}`)
      .then(({data})=>{
        this.list = data.data;
