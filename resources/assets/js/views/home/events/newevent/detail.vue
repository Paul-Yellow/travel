<template>
  <div v-loading.fullscreen.lock="isLoading" v-if="!isLoading">
    ...
    <div class="page-content-wrapper" >
       <div class="content-left-blk">
         <div class="total-nums-blk in-content-page">
           <p class="date"> 发布日期：{{newevent.created_at}} <br>更新日期：{{newevent.updated_at}} </p>
           <p class="icon-view">{{newevent.view_count}}</p>
          </div>  
          <div class="news-detail-blk">
            ...
            <h5 style="text-align:center;margin-bottom:1rem;">活动简介:&nbsp;&nbsp;{{newevent.content_short}}</h5>
            <dl class="event-info-list"> 
              <dt class="info-label"> 活动时间 </dt> 
              <dd class="info"> {{newevent.date}} </dd>
              <dt class="info-label"> 活动地点 </dt>
              <dd class="info"> 
                <a :href="`https://www.google.com/maps/place/${newevent.lo},${newevent.ln}`" class="btn-location-link" target="_blank" :title="newevent.area">{{newevent.area}}</a>
              </dd>
            </dl>
            <div class="manual-script-blk" v-html="newevent.content">
            </div>
            <share :title="newevent.title" :url="currentUrl" :image="newevent.imgUrl"></share>
          </div>
           <div class="page-func-blk">
             <router-link to="/newevent" class="btn-page-direction" target="_self" title="">回列表</router-link>
            </div>
        </div> 
        <div class="content-right-blk"> 
          <div class="right-side-info-blk"> <h3 class="title">近期最新活动</h3>
            ...
            <router-link to="/newevent" class="btn-more" title="更多活动" style="width:auto;">更多活动</router-link>
          </div>
    ...
    <comment :list="comment_list" api="newevents"></comment>
  </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods:{
    initData(){
        this.isLoading = true;
        this.$http.get(`/newevent/detail/${this.currentPage}`)
          .then(({data})=>{
            if(data.responData.status === 200){
              this.newevent = data.responData.newevents;
