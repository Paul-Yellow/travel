        ...
        <li class="item news" v-for="(item,index) in dataList" :key="index">
          <div class="info-card-item">
            ...
            <div class="func-blk">
              ...
              <favourite :item="item" type="favorited" api="/admin/newevents/favourite"></favourite>
              <links :item="item" type="liked" api="/admin/newevents/links"></links>
            </div>
          </div>
        </li>
      </ul>
...
export default {
  ...
  methods:{
    handleSizeChange(limit){
     this.$router.push({ path:'/newevent', query: { limit } })
    },
    handleCurrentChange(page){
      this.$router.push({ path:'/newevent', query: { page } })
    },
    fetchList(){
      this.listLoading = true;
      this.$http.get(`/newevent?page=${this.currentPage}&limit=${this.currentLimit}`)
      .then(({data})=>{
        this.list = data.data;
